...
#define DEQUE_BUF_SIZE 64

namespace nuts
{
    template <typename T, u64 Buf = DEQUE_BUF_SIZE>
    class deque
    {
        using pointer = T *;
    ...
    public:
        ...
        iterator begin()
        {
            ...
            // pointer cur = (first == (pointer)&map.front()[Buf - 1] + 1)
            //                   ? first
            //                   : (pointer)&map.front()[Buf - 1];
            pointer cur = first;
        ...
        iterator begin() const
        {
            ...
            // const_pointer cur =
            //     const_cast<pointer>((first != (pointer)&map.front()[Buf - 1] + 1)
            //                             ? first
            //                             : (pointer)&map.front()[Buf - 1]);
    ...
    {
        for (auto p = src.begin(); p != src.end() + 1; ++p)
            this->push_back(*p);
    }
    ...
    T &deque<T, Buf>::operator[](u64 _n)
    {
        ...
        u64 tail_len = end().get() - &map.back().front() + 1;
        u64 mid_len = Buf * (map.size() - 2);
        assert(_n < size());
        if (_n < head_len)
        {
