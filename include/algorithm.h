...
namespace nuts
{
	...
	bool is_sorted(Itr st, Itr ed, Comp cmp())
	{
		ed += 1;
	...
	template <Container _C, class Comp = nuts::less<typename _C::value_type>>
	bool is_sorted(const _C& x, Comp cmp())
	{
		auto ed = x.end() + 1;
	...
	// Return itr:none if not found && must be random_access -> O(logN)
	{
		...
		if (st >= cpy && st <= ed) return st;
		else
			return Itr {};
	}

	template <typename Itr, typename T>
	Itr binary_search(Itr st, Itr ed, const T& val)
	// Return false if not found -> O(logN)
	{
		return lower_bound(st, ed, val);
	}

	template <typename Seq, typename T>
	typename Seq::iterator binary_search(Seq& arr, const T& val)
